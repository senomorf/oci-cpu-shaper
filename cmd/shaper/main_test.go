...
func TestDefaultControllerFactory(t *testing.T) {
	t.Parallel()

	controller := defaultControllerFactory(modeEnforce)

	if controller.Mode() != modeEnforce {
		t.Fatalf("expected controller mode %q, got %q", modeEnforce, controller.Mode())
	}
}
